...
async fn main() -> Result<()> {
    ...
    let get_post_resp = client
        .get(&format!("{}/api/posts/{}/likes", BASE_URL, post_id))
        .send()
        .await?;
    print_response(get_post_resp).await?;

    println!("\n--- Test 5: Get users who liked the post ---");
    let get_likers_resp = client
        .get(&format!("{}/api/posts/{}/likers", BASE_URL, post_id))
        .send()
        .await?;
