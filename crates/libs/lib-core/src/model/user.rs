...
impl UserBmc {
    ...
    pub async fn request_password_reset(_ctx: &Ctx, mm: &ModelManager, email: &str) -> Result<()> {
        ...
        let reset_link = format!(
            "{}/reset?token={}",
            config.PASSWORD_RESET_BASE_URL, reset_token
        );
    ...
    pub async fn verify_email(_ctx: &Ctx, mm: &ModelManager, token: &str) -> Result<()> {
        ...
        if let Some(expires_at) = expires_at {
            ...
            if now > expires_at {
                tracing::error!(
                    "Verification token expired. Now: {}, Expires: {}",
                    now,
                    expires_at
                );
        ...
        query
            ...
            .values(vec![
                ...
                (
                    UserIden::EmailVerificationToken,
        ...
        tracing::info!(
            "Successfully verified email for user_id: {}. Rows affected: {}",
            user_id,
            count
        );
