...
use crate::model::{ModelManager, Error, Result};
...
impl JourneyBmc {
    ...
    async fn update_counter(
        ...
        match counter_type {
            CounterType::SavedCount => {
                query.value(
                    JourneyIden::SavedCount, Expr::col(JourneyIden::SavedCount).add(delta) 
                );
                // protect column from negative values
                if delta < 0 {
                    query.and_where(Expr::col(JourneyIden::SavedCount).gt(0));
                }
            }
            CounterType::ForwardCount => {
                query.value(
                    JourneyIden::ForwardCount, Expr::col(JourneyIden::ForwardCount).add(delta)
                );
                if delta < 0 {
            ...
            CounterType::TotalLikes => {
                query.value(
                    JourneyIden::TotalLikes,
                    Expr::col(JourneyIden::TotalLikes).add(delta),
                );
                if delta < 0 {
    ...
    pub async fn increment_saved_count(
        ctx: &Ctx,
        mm: &ModelManager,
        journey_id: &str,
    ) -> Result<()> {
        Self::update_counter(ctx, mm, journey_id, CounterType::SavedCount, 1).await
    }
    
    pub async fn decrement_saved_count(
        ctx: &Ctx,
        mm: &ModelManager,
        journey_id: &str,
    ) -> Result<()> {
    ...
    pub async fn increment_forward_count(
        ctx: &Ctx,
        mm: &ModelManager,
        journey_id: &str,
    ) -> Result<()> {
    ...
    pub async fn decrement_forward_count(
        ctx: &Ctx,
        mm: &ModelManager,
        journey_id: &str,
    ) -> Result<()> {
    ...
    pub async fn increment_total_likes(
        ctx: &Ctx,
        mm: &ModelManager,
        journey_id: &str,
        amount: i32,
    ...
    pub async fn decrement_total_likes(
        ctx: &Ctx,
        mm: &ModelManager,
        journey_id: &str,
        amount: i32,
