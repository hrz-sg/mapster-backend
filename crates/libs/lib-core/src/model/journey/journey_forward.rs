...
impl JourneyForwardBmc {
    pub async fn exists(ctx: &Ctx, mm: &ModelManager, journey_id: &str, user_id: &str) -> Result<bool> {
    ...
    pub async fn create_on_conflict(
        ...
        base::create_on_conflict::<Self, _, _>(
            ...
            journey_forward_c,
            &[
                JourneyForwardIden::JourneyId,
                JourneyForwardIden::UserId,
                JourneyForwardIden::ForwardToUserId,
                JourneyForwardIden::ForwardToChatId,
            ],
        )
        .await
    }
    ...
    pub async fn count(ctx: &Ctx, mm: &ModelManager, filter: Option<Vec<JourneyForwardFilter>>) -> Result<i64> {
